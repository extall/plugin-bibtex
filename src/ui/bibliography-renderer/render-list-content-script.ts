...
const joplin_resources = '.config/joplindev-desktop/resources'; // TODO: Replace before release!!!!! (Remove 'dev')
const homedir = require('os').homedir();
const fs = require("fs");

// TODO: When it will be possible to communicate with the plugin, we'll be able
// TODO: to load any CSL. For now, it will remain hardcoded here.
const csl_ieee_with_url = `
...
`;
const csl_apa_no_amp = `
<?xml version="1.0" encoding="utf-8"?>
<style xmlns="http://purl.org/net/xbiblio/csl" class="in-text" version="1.0" demote-non-dropping-particle="never" page-range-format="expanded">
  <info>
    <title>American Psychological Association 6th edition (no ampersand)</title>
    <title-short>APA (6th ed., no ampersand)</title-short>
    <id>http://www.zotero.org/styles/apa-6th-edition-no-ampersand</id>
    <link href="http://www.zotero.org/styles/apa-6th-edition-no-ampersand" rel="self"/>
    ...
    return {
        plugin: function (markdownIt, _options) {
            ...
            function references_list(state, silent) {
                console.log(state) ; // the bug below enters quickly stopping me from publishing this half ass contraption
            ...
            markdownIt.renderer.rules.references_body = function (tokens, index) {
                ...
                // Let's try to find the resources folder directly.
                let file_path:string = homedir.replace(/\\/g, "/") + "/" + joplin_resources + "/" + file + ".bib";
                let the_file: string;
                try{
                    the_file = readFileSync(file_path, 'utf8');
                }catch(e){
                    console.log("Could not read the resource file! More information below.");
                    console.log(e);
                }

                // Now we must read and parse the BibTeX file AGAIN
                // for rendering purposes using the desired CSL (default is IEEE)
                let the_parsed:Reference[] = parse(the_file);

                // Create a local datastore
                let myStore = new LocalStore(the_parsed);

                // Get unique IDs
                IDs = [...new Set(IDs)];

                // Create the CSL-based processing engine
                let citeproc = new CSL.Engine(
                    {
                        ...
                        retrieveItem: (refId: string) => {
                            return myStore.getReferenceById(refId);
                        },
                    },
                    csl_apa_no_amp
                );
                ...
                let the_refs = citeproc.makeBibliography()[1].join("<br />");
